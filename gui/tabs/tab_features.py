...
from widgets.tagged import add_combo_tagged

from widgets.helpers import create_master_switch_callback

from widgets.basic import add_float, add_int
from widgets.callbacks import update_class_ids_callback, update_config_callback
from widgets.crosshair_preview import generate_crosshair_preview_callback
from widgets.helpers import update_dependent_controls
from widgets.preview import update_search_bounds
from widgets.tagged import add_input_text_tagged, add_bool_tagged, add_int_tagged, add_float_input_tagged, \
    add_int_input_tagged, add_float_tagged
...
def build_features_tab(blue_notice_theme):
    with dpg.tab(label="功能参数"):
        ...
        with dpg.group(horizontal=True):
            dpg.add_text("当前参数组:")
            dpg.add_combo(
                items=["默认参数组", "步枪参数组", "狙击枪参数组", "冲锋枪参数组"],
                default_value="默认参数组",
                width=200,
                callback=lambda s, a: print(f"切换到: {a}")  # 后续实现参数组切换逻辑
            )
            dpg.add_button(label="新建", small=True)
            dpg.add_button(label="复制", small=True)
            dpg.add_button(label="删除", small=True)
        ...
        with dpg.tab_bar():
            ...
            with dpg.tab(label="视觉识别"):
                ...
                update_dependent_controls("IGNORE_SMALL_TARGET_HEAD", small_target_deps,
                                          ignore_small_head_enabled)
