...
def torch_safe_load(weight):
    ...
    from ultralytics.nn.modules import Conv, C2f, Bottleneck, BottleneckCSP, SPPF  # 添加更多模块
    ...
    try:
        ...
        # 回退到非安全加载
        try:
            LOGGER.warning(f"安全加载失败，尝试非安全加载: {e}")
            return torch.load(file, map_location='cpu', weights_only=False), file
        except ModuleNotFoundError as module_error:
            # 处理模块缺失错误
            if module_error.name == 'models':
                raise TypeError(
                    emojis(f'ERROR ❌️ {weight} appears to be an Ultralytics YOLOv5 model originally trained '
                           f'with https://github.com/ultralytics/yolov5.\nThis model is NOT forwards compatible with '
                           f'YOLOv8 at https://github.com/ultralytics/ultralytics.'
                           f"\nRecommend fixes are to train a new model using the latest 'ultralytics' package or to "
                           f"run a command with an official YOLOv8 model, i.e. 'yolo predict model=yolov8n.pt'")) from module_error
            LOGGER.warning(
                f"WARNING ⚠️ {weight} appears to require '{module_error.name}', which is not in ultralytics requirements."
                f"\nAutoInstall will run now for '{module_error.name}' but this feature will be removed in the future."
                f"\nRecommend fixes are to train a new model using the latest 'ultralytics' package or to "
                f"run a command with an official YOLOv8 model, i.e. 'yolo predict model=yolov8n.pt'")
            check_requirements(module_error.name)  # install missing module
